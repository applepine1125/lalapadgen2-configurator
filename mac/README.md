...
起動から数秒後と、メニューの「Lala2Conf > 更新を確認…」で GitHub Releases(タグ `lala2conf-b<ビルド番号>`)を確認し、手元より新しいビルドがあれば通知します。「更新する」を選ぶと zip と署名(`.sig`)をダウンロードし、**埋め込んだ公開鍵で署名を検証してから**バンドルを置き換えて再起動します。署名が無い・合わない場合は入れ替えずに中止します。
...
- **Bluetooth の許可ダイアログを誤って拒否した**: 「システム設定 > プライバシーとセキュリティ > Bluetooth」でこのアプリ(Lala2Conf)を有効にしてから再起動してください。
